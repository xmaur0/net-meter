...
#define INITGUID // faz os GUIDs do ETW serem definidos aqui mesmo, sem
                 // precisar linkar nenhuma lib extra pra isso
...
// Carregada dinamicamente (via LoadLibrary/GetProcAddress) para não depender
// de linkar dwmapi.lib no compilador/script de build de quem for compilar.
...
// Back-buffer persistente: o conteúdo é desenhado aqui só quando os dados
// realmente mudam (RenderWidget). WM_PAINT / WM_ERASEBKGND / WM_PRINTCLIENT
// apenas copiam (blit) o que já está pronto aqui — isso garante que a janela
// NUNCA fica sem conteúdo desenhado, não importa quantas vezes o Windows
// mande repintar (o que acontece com frequência por ser filha da taskbar).
HDC     g_hdcMem  = NULL;
...
// ============================================================================
// MEDIÇÃO REAL DE REDE POR PROCESSO (ETW - Event Tracing for Windows)
// ============================================================================
// A versão antiga "adivinhava" qual processo estava usando a rede olhando
// só quem tinha mais conexões TCP abertas - o que é impreciso (ex: um
// torrent usa UDP, que nem era contado, então o crédito ia sempre pro
// processo com mais conexões TCP, tipo o navegador).
//
// Esta versão usa o provedor de kernel "TcpIp"/"UdpIp" do ETW, que entrega
// eventos de envio/recebimento reais com PID e tamanho em bytes - a mesma
// técnica usada por ferramentas como GlassWire/Process Explorer. Só funciona
// executando como Administrador (exigência do próprio Windows para eventos
// de kernel), então há um fallback caso não seja possível.
...
// GUIDs fixos das classes de evento "TcpIp" e "UdpIp" do Windows (MOF clássico).
...
std::map<DWORD, ULONG64> g_etwBytesIn;      // acumulado no intervalo atual, por PID (zerado a cada leitura via swap)
...
std::vector<BYTE> g_etwPropsBuf; // precisa ficar viva enquanto a sessão existir
...
// Chamada pelo ETW (em uma thread própria, dedicada só a consumir eventos)
// toda vez que o kernel reporta um envio/recebimento de rede. Fica o mais
// enxuta possível (sem alocação, só um memcpy + soma), já que pode ser
// chamada centenas de vezes por segundo em conexões pesadas.
void WINAPI EtwEventCallback(PEVENT_RECORD pEvent) {
    ...
    // IPv6: Send=26, Receive=27 - códigos DIFERENTES, documentados em
    // TcpIp_SendIPV6/TcpIp_TypeGroup3 e UdpIp_TypeGroup2. Sem isso, todo
    // tráfego IPv6 (comum hoje em dia) era ignorado silenciosamente.
    ...
    // Layout MOF clássico (TcpIp/UdpIp, v4 e v6): os dois primeiros campos
    // são sempre PID (4 bytes) e tamanho da transferência (4 bytes) -
    // confirmado na documentação oficial (TcpIp_SendIPV4, TcpIp_TypeGroup1,
    // UdpIp_TypeGroup1/2), o mesmo layout vale pra v4 e v6.
...
// Inicia a sessão de rastreamento em tempo real. Só é chamada quando a janela
// de processos é aberta (não no início do programa) e é desligada quando a
// janela fecha - assim não gasta CPU/memória à toa quando ninguém está
// olhando a tela de processos.
bool StartEtwMonitoring() {
    if (g_etwState == ETW_RUNNING) return true;
    if (g_etwState == ETW_FAILED_NO_ADMIN) return false; // já sabemos que não vai dar, não tenta de novo
    ...
    // Só pode existir UMA sessão "NT Kernel Logger" no sistema todo por vez.
    // Para qualquer uma que já esteja rodando (de uma execução anterior que
    // não fechou direito, outra ferramenta, etc.) pra garantir que a nossa
    // consiga habilitar os eventos de rede.
    {
    ...
    props->Wnode.ClientContext = 1; // resolução de timestamp: QPC
    props->Wnode.Flags = WNODE_FLAG_TRACED_GUID;
    props->LogFileMode = EVENT_TRACE_REAL_TIME_MODE;
    props->EnableFlags = EVENT_TRACE_FLAG_NETWORK_TCPIP;

    // Buffers generosos: torrent/P2P gera uma enxurrada de pacotes pequenos
    // (UDP/uTP) por segundo. Com o buffer padrão (pequeno), a sessão descarta
    // eventos do fluxo "menor" quando um fluxo muito maior enche o buffer
    // primeiro - e é exatamente isso que fazia só o maior consumidor aparecer
    // e todo o resto cair pra 0. Buffers maiores/mais numerosos evitam essa
    // perda. O custo é só memória (cada buffer soma alguns KB, liberado ao
    // fechar a janela de processos).
    props->BufferSize = 64;       // KB por buffer
    props->MinimumBuffers = 32;
    props->MaximumBuffers = 128;
    props->FlushTimer = 1;        // segundos
    ...
    g_etwThread = std::thread([consumerHandleCopy]() {
        ...
        ProcessTrace(&h, 1, NULL, NULL); // bloqueia consumindo eventos até CloseTrace ser chamado
    });
...
// Cache dos nomes de processo (PID -> nome do executável). Evita chamar
// OpenProcess/QueryFullProcessImageNameW pra tudo a cada 800ms - o nome de
// um processo não muda, então só precisamos resolver uma vez por PID. O
// cache é limitado e limpo periodicamente pra não crescer sem limite e pra
// não confundir nomes se um PID for reciclado pelo Windows depois que o
// processo original fechar.
...
std::wstring ResolveProcessName(DWORD pid) {
    ...
    // Limpa o cache a cada 60s (limita memória e evita nomes desatualizados
    // por reaproveitamento de PID).
    ...
    // Limite defensivo de tamanho do cache (processos de vida curta indo e
    // vindo não devem fazer isso crescer indefinidamente entre limpezas).
...
// Chamado imediatamente quando o foreground muda ou uma janela do sistema
// (como a barra de tarefas) reordena seu Z-order. Usado apenas como FALLBACK,
// caso não seja possível reparentar a janela para dentro da taskbar (ver
// TryReparentToTaskbar). No Windows 11 puro reparentar resolve o problema
// de raiz; este hook é só uma rede de segurança.
...
// Hook de baixo nível: roda no exato instante de qualquer clique do mouse na
// tela, ANTES do Explorer terminar de processar o clique na taskbar. Reforçar
// o topmost aqui é mais rápido que esperar a notificação de reordenação de
// janelas (que passa por uma fila do subsistema de acessibilidade e tem um
// atraso perceptível).
...
// Tenta transformar a janela em FILHA da própria barra de tarefas (Shell_TrayWnd).
// Isso é o que realmente resolve o piscar no Windows 11: em vez de competir
// por "topmost" contra a camada de composição da taskbar, a janela passa a
// fazer parte da mesma árvore de janelas e é desenhada junto, sem briga de Z-order.
// Se falhar por qualquer motivo (bloqueio do sistema, versão diferente, etc.),
// devolve os estilos originais e retorna false, caindo no modo antigo (topmost).
// Define a taskbar como "dona" (owner) da nossa janela, SEM reparentar de
// verdade (a janela continua independente, com sua própria superfície de
// composição do DWM - por isso continua 100% opaca). Uma janela com owner
// é automaticamente mantida pelo Windows logo acima do seu dono na ordem Z,
// o que ajuda a evitar a disputa de topmost quando a taskbar é clicada,
// sem sofrer o problema de mistura visual (Mica) que a reparentagem causava.
...
bool TryReparentToTaskbar(HWND hwnd) {
    ...
    if (SetParent(hwnd, hShellTray) == NULL) {
        // Falhou: reverte tudo para o comportamento antigo (janela topmost solta)
...
void AlignToTaskbar(HWND hwnd) {
    ...
    if (g_reparented && g_hShellTray) {
        // Janela é filha da taskbar: posição precisa ser relativa ao client
        // area do pai, não à tela.
        POINT pt = { posX, posY };
    ...
    // Modo fallback (não foi possível reparentar): janela topmost solta
    if (posX != s_lastX || posY != s_lastY || g_rectWidth != s_lastW || RECT_HEIGHT != s_lastH) {
        ...
        // Só garante que continua topmost, sem mover/redesenhar/mostrar de novo
...
// Desenha o conteúdo atual (velocidades + gráfico) no back-buffer persistente.
// Chamado só quando os dados mudam de verdade (a cada tick do timer ou troca
// de unidade) — NUNCA a partir de WM_PAINT/ERASEBKGND/PRINTCLIENT, senão o
// histórico do gráfico avançaria rápido demais toda vez que o Windows manda
// repintar por conta própria.
void RenderWidget() {
    EnsureBackBuffer();
    HDC hdcMem = g_hdcMem;
    ...
    if (!g_hFont) return; // ainda não inicializado (primeiro WM_CREATE)
...
// Só copia o que já está pronto no back-buffer. Usado por WM_PAINT,
// WM_ERASEBKGND e WM_PRINTCLIENT — assim a janela sempre mostra conteúdo
// opaco, não importa qual dessas três mensagens o Windows mandar.
...
void DisplaySimpleResultModal(SpeedTestResultData* pData) {
    ...
    // Ajuste manual: nos testes daqui, o resultado bruto do speedtest ficava
    // ~100 Mega acima do plano real contratado - então descontamos 100 Mega
    // sempre do resultado final exibido (nunca deixando ficar negativo).
...
std::wstring GetProcessNetworkText() {
    // Conexões TCP abertas por PID - agora usado só pra exibir a coluna
    // "CONEXÕES" e pra listar processos com conexão ativa mesmo com 0 bytes
    // no instante da amostra. NÃO é mais usado pra decidir quem "ganha" o
    // tráfego (isso vem de medição real via ETW agora).
    ...
    if (etwOk) {
        ...
        if (elapsed < 0.05) elapsed = 0.05; // evita divisão por ~0 se chamado rápido demais
        g_etwPrevSampleTime = now;

        // swap() em vez de cópia: é O(1) (só troca ponteiros internos), não
        // O(n). Com o torrent gerando milhares de eventos por segundo numa
        // thread separada, uma cópia (O(n), mais lenta) segurando a trava
        // criava disputa que chegava a travar a thread da interface (e por
        // tabela o timer da janelinha principal também, já que é tudo a
        // mesma thread) - isso que fazia até o TOTAL parecer travado.
        // O swap já entrega exatamente os bytes acumulados NESTE intervalo
        // e zera o acumulador pro próximo, então nem precisa mais guardar
        // um "snapshot anterior" pra subtrair.
        ...
        for (auto& kv : pidsToShow) {
            ...
            if (down < 1.0 && up < 1.0 && conns == 0) continue; // ignora ruído/PID morto
        ...
        // Fallback sem privilégio de administrador: mostra só a lista de
        // conexões ativas, deixando claro que não é medição real de bytes.
    ...
    std::wstring msg;
    if (etwOk) {
        msg += L" USO DE REDE POR PROCESSO - MEDIÇÃO REAL (ETW) - Atualizado a cada 800ms\r\n";

        // Diagnóstico: se mesmo com os buffers maiores ainda estiver
        // perdendo eventos sob rajada muito pesada de tráfego, avisa aqui
        // em vez de deixar os números incorretos silenciosamente.
        // Existem DOIS contadores de perda diferentes no ETW: "EventsLost"
        // (perda ao gravar o evento, por falta de buffer) e
        // "RealTimeBuffersLost" (perda na ENTREGA em tempo real pro nosso
        // consumidor - o mais relevante pra esse cenário, e o que a versão
        // anterior deste código não checava).
        if (!g_etwPropsBuf.empty()) {
            ...
            if (ControlTraceW(g_etwSessionHandle, kKernelLoggerName, qprops, EVENT_TRACE_CONTROL_QUERY) == ERROR_SUCCESS) {
                if (qprops->EventsLost > 0 || qprops->RealTimeBuffersLost > 0) {
                    msg += L" (aviso: " + std::to_wstring(qprops->EventsLost) +
                        L" eventos perdidos / " + std::to_wstring(qprops->RealTimeBuffersLost) +
                        L" buffers de entrega perdidos - números podem ficar imprecisos)\r\n";
                }
            }
        }
...
LRESULT CALLBACK ProcListWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Liga a medição real (ETW) só enquanto essa janela está aberta -
        // evita gastar CPU/memória com isso em segundo plano quando
        // ninguém está olhando a tela de processos.
        StartEtwMonitoring();
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ...
        // Reparentagem para dentro da taskbar DESATIVADA: no Windows 11 ela
        // resolvia o piscar, mas o DWM (efeito Mica da taskbar) faz nosso
        // conteúdo parecer transparente, e isso não dá pra corrigir por fora.
        // Voltamos ao modo janela independente (sempre opaca).
        g_reparented = false;
        AlignToTaskbar(hwnd);
        ...
        RenderWidget(); // garante que o back-buffer já tem conteúdo válido

        // Desliga qualquer material de fundo (Mica/acrílico) que o DWM possa
        // tentar aplicar na nossa janela por ela estar dentro da taskbar.
        DisableDwmBackdrop(hwnd);

        if (!g_reparented) {
            // Fallback: hooks para reagir na hora a mudancas de foreground /
            // reordenacao de janelas do sistema.
    ...
    case WM_ERASEBKGND: {
        // Como a janela agora e filha da taskbar, o Windows manda essa
        // mensagem toda vez que a taskbar (com efeito Mica/acrilico) se
        // redesenha. Em vez de só preencher com uma cor solida, copiamos o
        // conteudo real do back-buffer - assim nunca aparece "vazado".
        BlitWidget((HDC)wParam);
        return 1;
    }

    case WM_PRINTCLIENT: {
        // Janelas host que usam composicao (como a taskbar do Win11) as
        // vezes pedem pra janela filha "se imprimir" em vez de mandar
        // WM_PAINT. Se a gente nao trata isso, o conteudo nao aparece.
        BlitWidget((HDC)wParam);
        return 1;
    }
    ...
    case WM_RBUTTONUP: {
        ...
        case ID_TRAY_AUTOSTART:
            ...
            break;
        case ID_TRAY_EXIT:
            // Fecha a janela de processos primeiro (se estiver aberta), pra
            // garantir que o WM_DESTROY dela rode e desligue a sessão do ETW
            // direito - senão ela ficaria orfã rodando no sistema mesmo
            // depois do programa fechar.
    ...
    default:
        if (msg == g_uTaskbarCreatedMsg && g_uTaskbarCreatedMsg != 0) {
            // Explorer reiniciou (crash, "Reiniciar" pelo Gerenciador de
            // Tarefas, etc.) - realinha a janela com a nova taskbar.
            AlignToTaskbar(hwnd);
            SetTaskbarAsOwner(hwnd);
            return 0;
        }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Rede de segurança final: garante que a sessão do ETW (que é do sistema
    // inteiro, não só do nosso processo) nunca fique rodando órfã, mesmo que
    // o programa termine por um caminho que não passou pelo WM_DESTROY da
    // janela de processos (ex: logoff/desligamento do Windows).
    StopEtwMonitoring();
