...
#define INITGUID // makes the ETW GUIDs get defined right here
...
// Temas de Cores
#define ID_THEME_LIGHT     1030
#define ID_THEME_DARK      1031
#define ID_THEME_CYBER     1032
#define ID_THEME_MATRIX    1033
#define ID_THEME_DARKMATTER 1034
...
enum UITheme { THEME_LIGHT = 0, THEME_DARK, THEME_CYBER, THEME_MATRIX, THEME_DARKMATTER };

Language g_language = LANG_PT;
UIScale  g_scale    = SCALE_NORMAL;
UITheme  g_theme    = THEME_LIGHT;

// Fator global de redução de 30% aplicado a todas as escalas
const double K_SCALE_REDUCTION = 0.70;

// Variáveis dinâmicas de dimensionamento (tamanho ATUAL, usado para desenhar/posicionar)
int g_rectWidth = 71;
int g_rectHeight = 31;
int g_fontSizeWidget = -11;

// -----------------------------------------------------------------------
// FIX: tamanho "canônico" calculado apenas por UpdateScaleDimensions().
// AlignToTaskbar() NUNCA deve alterar o tamanho permanentemente a partir
// de dados possivelmente inválidos/zerados vindos do Shell. Guardamos
// aqui os valores corretos para poder sempre restaurá-los.
// -----------------------------------------------------------------------
int g_rectWidthBase = 71;
int g_rectHeightBase = 31;

// Menor largura de taskbar que consideramos plausível para confiar no
// valor retornado por SHAppBarMessage (evita usar rc={0,0,0,0} por engano).
const int kMinPlausibleTaskbarExtent = 40;
...
// -----------------------------------------------------------------------
// Deteccao de jogo/app em tela cheia + reducao de CPU dos hooks globais.
// -----------------------------------------------------------------------
bool g_widgetOculto = false;                 // true quando escondido por causa de fullscreen
ULONGLONG g_ultimoReposicionamento = 0;      // throttle dos hooks (GetTickCount64)
const ULONGLONG kThrottleHooksMs = 150;      // intervalo minimo entre SetWindowPos via hook
...
// Map de Dicionário Multilingue Ampliado
std::map<std::wstring, std::map<Language, std::wstring>> g_dict = {
    ...
    {L"THEME_MENU", {
        {LANG_PT, L"Temas de Cores"}, {LANG_EN, L"Color Themes"}, {LANG_ES, L"Temas de Color"},
        {LANG_FR, L"Thèmes de Couleur"}, {LANG_DE, L"Farbthemen"}, {LANG_IT, L"Temi di Colore"},
        {LANG_RU, L"Цветовые темы"}, {LANG_JA, L"カラーテーマ"}, {LANG_ZH, L"颜色主题"}, {LANG_KO, L"색상 테마"}
    }},
    {L"THEME_LIGHT_TXT", {{LANG_PT, L"Tema Claro"}, {LANG_EN, L"Light Theme"}, {LANG_ES, L"Tema Claro"}}},
    {L"THEME_DARK_TXT", {{LANG_PT, L"Tema Escuro (D:Laranja / U:Amarelo)"}, {LANG_EN, L"Dark Theme (D:Orange / U:Yellow)"}, {LANG_ES, L"Tema Oscuro"}}},
    {L"THEME_CYBER_TXT", {{LANG_PT, L"Azul Neon (Cyberpunk)"}, {LANG_EN, L"Neon Blue (Cyberpunk)"}, {LANG_ES, L"Azul Neón"}}},
    {L"THEME_MATRIX_TXT", {{LANG_PT, L"Matrix (Verde Hacker)"}, {LANG_EN, L"Matrix (Hacker Green)"}, {LANG_ES, L"Matrix Verde"}}},
    {L"THEME_DARKMATTER_TXT", {{LANG_PT, L"Matéria Escura (Roxo/Ciano)"}, {LANG_EN, L"Dark Matter (Purple/Cyan)"}, {LANG_ES, L"Materia Oscura"}}},
    {L"LANG_MENU", {
...
// Aplicar a paleta de cores do Tema Selecionado
void ApplyThemeColors() {
    ...
    switch (g_theme) {
    case THEME_DARK:
        g_hBgBrush  = CreateSolidBrush(RGB(0, 0, 0));            // Preto Puro
        g_hPenGreen = CreatePen(PS_SOLID, 1, RGB(255, 140, 0));  // Alaranjado para Grafico D
        g_hPenRed   = CreatePen(PS_SOLID, 1, RGB(255, 255, 0));  // Amarelo para Grafico U
        break;
    case THEME_CYBER:
        g_hBgBrush  = CreateSolidBrush(RGB(10, 15, 30));         // Azul Noturno Profundo
        g_hPenGreen = CreatePen(PS_SOLID, 1, RGB(0, 229, 255));  // Ciano Neon
        g_hPenRed   = CreatePen(PS_SOLID, 1, RGB(255, 0, 128));  // Magenta Neon
        break;
    case THEME_MATRIX:
        g_hBgBrush  = CreateSolidBrush(RGB(5, 20, 5));           // Verde Musgo Escuro
        g_hPenGreen = CreatePen(PS_SOLID, 1, RGB(0, 255, 65));   // Verde Matrix
        g_hPenRed   = CreatePen(PS_SOLID, 1, RGB(180, 255, 0));  // Verde Limão
        break;
    case THEME_DARKMATTER:
        g_hBgBrush  = CreateSolidBrush(RGB(20, 10, 25));         // Roxo Escuro
        g_hPenGreen = CreatePen(PS_SOLID, 1, RGB(186, 85, 211)); // Orquídea
        g_hPenRed   = CreatePen(PS_SOLID, 1, RGB(0, 255, 200));  // Turquesa
        break;
    case THEME_LIGHT:
    default:
        g_hBgBrush  = CreateSolidBrush(RGB(220, 220, 220));      // Cinza Claro Padrão
        g_hPenGreen = CreatePen(PS_SOLID, 1, RGB(0, 130, 0));    // Verde Escuro
        g_hPenRed   = CreatePen(PS_SOLID, 1, RGB(200, 0, 0));    // Vermelho Escuro
        break;
    }
}
...
void SaveSettings() {
    ...
    if (RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\NetSpeedTray", 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        ...
        RegSetValueExW(hKey, L"Theme", 0, REG_DWORD, (BYTE*)&theme, sizeof(theme));
        RegCloseKey(hKey);
    }
}
...
void LoadSettings() {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\NetSpeedTray", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        DWORD lang = 0, scale = 0, unit = 0, theme = 0, sz = sizeof(DWORD);
        ...
        if (RegQueryValueExW(hKey, L"Theme", NULL, NULL, (LPBYTE)&theme, &sz) == ERROR_SUCCESS) g_theme = (UITheme)theme;
        RegCloseKey(hKey);
    }
}
...
void CALLBACK WinEventProc(HWINEVENTHOOK, DWORD event, HWND, LONG idObject, LONG, DWORD, DWORD) {
    ...
    // Enquanto o widget estiver escondido (jogo em tela cheia ativo), nao vale a
    // pena gastar CPU tentando manter topmost — o widget nem esta visivel.
    if (g_widgetOculto) return;

    ULONGLONG agora = GetTickCount64();
    if (agora - g_ultimoReposicionamento < kThrottleHooksMs) return;
    g_ultimoReposicionamento = agora;
...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        if (wParam == WM_LBUTTONDOWN || wParam == WM_RBUTTONDOWN ||
            ...
            if (g_hwndMain && !g_reparented && !g_widgetOculto) {
                ULONGLONG agora = GetTickCount64();
                if (agora - g_ultimoReposicionamento >= kThrottleHooksMs) {
                    g_ultimoReposicionamento = agora;
                    SetWindowPos(g_hwndMain, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
                }
            }
...
// -----------------------------------------------------------------------
// FIX: esta é agora a ÚNICA função que decide o tamanho "correto" do
// widget (g_rectWidthBase/g_rectHeightBase), a partir da escala escolhida
// pelo usuário. AlignToTaskbar() só deve usar/replicar esses valores (ou,
// no caso de taskbar vertical, ajustar com dados validados) — nunca deixar
// o widget "grudado" num tamanho errado por causa de uma falha temporária
// do Shell.
// -----------------------------------------------------------------------
void UpdateScaleDimensions() {
    ...
    g_rectWidthBase  = (int)(102 * factor);
    g_rectHeightBase = (int)(45 * factor);

    // Sincroniza o tamanho atual com o tamanho canônico recém-calculado.
    g_rectWidth  = g_rectWidthBase;
    g_rectHeight = g_rectHeightBase;
...
// -----------------------------------------------------------------------
// FIX principal do bug relatado: a função reposiciona o widget, e SÓ
// ajusta a LARGURA (nunca a altura) no caso especial de taskbar vertical
// (ABE_LEFT/ABE_RIGHT), e apenas quando SHAppBarMessage teve sucesso E
// o valor retornado é plausível (> kMinPlausibleTaskbarExtent). Em
// qualquer outro cenário (falha da API, taskbar horizontal, valor
// implausível) o tamanho é sempre restaurado para g_rectWidthBase /
// g_rectHeightBase, então o widget nunca fica "preso" pequeno.
// -----------------------------------------------------------------------
void AlignToTaskbar(HWND hwnd) {
    APPBARDATA abd = { sizeof(APPBARDATA) };
    BOOL ok = SHAppBarMessage(ABM_GETTASKBARPOS, &abd);
    ...
    // Sempre parte do tamanho canônico; só é sobrescrito abaixo se os
    // dados da taskbar vertical forem válidos.
    g_rectWidth  = g_rectWidthBase;
    g_rectHeight = g_rectHeightBase;

    int posX = 0, posY = 0;

    // Se a chamada falhou, não temos como saber a orientação/posição da
    // taskbar agora. Mantém o tamanho canônico e não mexe na posição
    // (evita "sumir" a janela em 0,0 com base em dados zerados).
    if (!ok) {
        return;
    }
    ...
    case ABE_RIGHT: {
        ...
        // Só confia no valor da taskbar se ele for plausível; caso
        // contrário mantém g_rectWidthBase (já setado acima).
        if (taskbarWidth >= kMinPlausibleTaskbarExtent) {
...
// -----------------------------------------------------------------------
// Detecta se a janela em primeiro plano esta em modo tela cheia (jogo
// exclusivo/borderless, ou qualquer janela cobrindo 100% do monitor).
// Ignora a propria janela do widget e janelas do proprio Shell do Windows
// (area de trabalho, taskbar, etc), que tambem costumam "cobrir" o monitor.
// -----------------------------------------------------------------------
bool JanelaEmTelaCheia(HWND hwndForeground, HWND hwndSelf) {
    if (!hwndForeground || hwndForeground == hwndSelf) return false;
    if (!IsWindow(hwndForeground) || !IsWindowVisible(hwndForeground)) return false;
    if (IsIconic(hwndForeground)) return false; // minimizada nao conta

    wchar_t className[256] = { 0 };
    GetClassNameW(hwndForeground, className, 255);

    // Janelas do proprio Shell do Windows nunca contam como "jogo em tela cheia".
    ...
    if (wcscmp(className, L"Shell_SecondaryTrayWnd") == 0) return false;
    if (wcscmp(className, L"Windows.UI.Core.CoreWindow") == 0) return false; // Start Menu, Action Center, etc.

    RECT rcWin = { 0 };
    if (!GetWindowRect(hwndForeground, &rcWin)) return false;

    HMONITOR hMon = MonitorFromWindow(hwndForeground, MONITOR_DEFAULTTONEAREST);
    MONITORINFO mi = { sizeof(MONITORINFO) };
    if (!GetMonitorInfoW(hMon, &mi)) return false;

    // Considera "tela cheia" quando a janela cobre a area inteira do monitor
    // (cobre igual ou mais, evita falhar por 1px de diferenca de borda).
    bool cobreMonitor =
        rcWin.left   <= mi.rcMonitor.left  &&
...
void UpdateNetworkData(HWND hwnd) {
    // -------------------------------------------------------------------
    // Verifica tela cheia primeiro. Se um jogo/app estiver cobrindo o
    // monitor, esconde o widget e PULA todo o resto do trabalho deste
    // tick (sem realinhar taskbar, sem redesenhar bitmap, sem invalidar
    // janela) — é exatamente aqui que a CPU é economizada durante o jogo.
    // -------------------------------------------------------------------
    bool telaCheiaAgora = JanelaEmTelaCheia(GetForegroundWindow(), hwnd);
    ...
    if (telaCheiaAgora) {
        ...
        return; // widget ja escondido: nada mais a fazer neste tick
    }

    if (g_widgetOculto) {
        // Saiu do modo tela cheia: reexibe e realinha antes de continuar.
        g_widgetOculto = false;
        ShowWindow(hwnd, SW_SHOWNOACTIVATE);
        SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);

        // Reseta a base de amostragem de rede: como ficamos parados (possivelmente
        // por horas, durante o jogo), calcular velocidade contra o g_prevTime antigo
        // geraria um numero de KB/s completamente distorcido neste primeiro tick.
        // Em vez disso, so recomeca a contagem a partir de agora; a velocidade real
        // volta a aparecer corretamente a partir do proximo tick (900ms depois).
        ULONG64 baseIn = 0, baseOut = 0;
        GetTotalNetworkTraffic(baseIn, baseOut);
        g_prevInBytes = baseIn;
        g_prevOutBytes = baseOut;
...
void RenderWidget() {
    ...
    // Definição de Cores das Letras baseadas no Tema
    COLORREF colorD = RGB(30, 30, 30);
    COLORREF colorU = RGB(180, 0, 0);

    if (g_theme == THEME_DARK) {
        colorD = RGB(255, 140, 0); // Fonte Alaranjada para o D
        colorU = RGB(255, 255, 0); // Fonte Amarela para o U
    } else if (g_theme == THEME_CYBER) {
        colorD = RGB(0, 229, 255);
        colorU = RGB(255, 0, 128);
    } else if (g_theme == THEME_MATRIX) {
        colorD = RGB(0, 255, 65);
        colorU = RGB(180, 255, 0);
    } else if (g_theme == THEME_DARKMATTER) {
        colorD = RGB(186, 85, 211);
        colorU = RGB(0, 255, 200);
    }

    SetTextColor(hdcMem, colorD);
    wchar_t txtDown[20];
...
std::wstring GetProcessNetworkText() {
    ...
    for (auto& proc : list) {
        ...
        // Alinhamento das colunas ajustado (%10s ajustado para encaixar exatamente sob os cabeçalhos)
        wchar_t line[300];
        StringCchPrintfW(line, 300, L" %-28s | %-5lu | %-11d | %10s | %10s\r\n", 
...
LRESULT CALLBACK ProcListWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        StartEtwMonitoring();
        // Atualização ajustada para 970ms conforme solicitado
        SetTimer(hwnd, ID_TIMER_PROC, 970, NULL);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_RBUTTONUP: {
        ...
        // Submenu Temas de Cores
        AppendMenuW(hThemeMenu, MF_STRING | (g_theme == THEME_LIGHT      ? MF_CHECKED : MF_UNCHECKED), ID_THEME_LIGHT,      Tr(L"THEME_LIGHT_TXT").c_str());
        AppendMenuW(hThemeMenu, MF_STRING | (g_theme == THEME_DARK       ? MF_CHECKED : MF_UNCHECKED), ID_THEME_DARK,       Tr(L"THEME_DARK_TXT").c_str());
        AppendMenuW(hThemeMenu, MF_STRING | (g_theme == THEME_CYBER      ? MF_CHECKED : MF_UNCHECKED), ID_THEME_CYBER,      Tr(L"THEME_CYBER_TXT").c_str());
        AppendMenuW(hThemeMenu, MF_STRING | (g_theme == THEME_MATRIX     ? MF_CHECKED : MF_UNCHECKED), ID_THEME_MATRIX,     Tr(L"THEME_MATRIX_TXT").c_str());
        AppendMenuW(hThemeMenu, MF_STRING | (g_theme == THEME_DARKMATTER ? MF_CHECKED : MF_UNCHECKED), ID_THEME_DARKMATTER, Tr(L"THEME_DARKMATTER_TXT").c_str());
        ...
        AppendMenuW(hMenu, MF_POPUP, (UINT_PTR)hThemeMenu, Tr(L"THEME_MENU").c_str());
        ...
        case ID_THEME_DARK:
        case ID_THEME_CYBER:
        case ID_THEME_MATRIX:
        case ID_THEME_DARKMATTER:
            if (cmd == ID_THEME_LIGHT) g_theme = THEME_LIGHT;
            else if (cmd == ID_THEME_DARK) g_theme = THEME_DARK;
            else if (cmd == ID_THEME_CYBER) g_theme = THEME_CYBER;
            else if (cmd == ID_THEME_MATRIX) g_theme = THEME_MATRIX;
            else if (cmd == ID_THEME_DARKMATTER) g_theme = THEME_DARKMATTER;
            ApplyThemeColors();
            SaveSettings();
            RenderWidget();
            InvalidateRect(hwnd, NULL, FALSE);
